...
export default function SubscriptionPage() {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const { order_id, amount, key_id, user_name, user_email, simulation } = orderRes.data;

      // Handle simulated payment mode seamlessly without launching real Razorpay checkout
      if (simulation || key_id === 'rzp_test_simulation') {
        setPaymentStep(2);
        await axios.post(
