...
db = client[os.environ.get('DB_NAME', 'giginsure')]
...
def get_jwt_secret():
    return os.environ.get("JWT_SECRET", "giginsure_super_secret_jwt_key_2026")
...
async def create_razorpay_order(req: RazorpayOrderRequest, request: Request):
    ...
    if not _rzp_key_id:
        # Simulation mode when Razorpay credentials are not provided
        return {
            "order_id":  f"order_sim_{secrets.token_hex(6)}",
            "amount":    amount_paise,
            "currency":  "INR",
...
async def verify_razorpay_payment(req: RazorpayVerifyRequest, request: Request):
    ...
    user = await get_current_user(request)
    is_simulation = req.razorpay_order_id.startswith("order_sim_") or not _rzp_key_secret
...
async def startup():
    ...
    try:
        ...
        logger.warning(f"MongoDB connection notice during startup: {e}")
    
    # Write test credentials (best-effort — path may not exist in all hosting envs)
    try:
...
async def health_check():
    ...
    return {"status": "ok", "service": "GigInsure API"}

@app.get("/")
async def root():
    return {"status": "ok", "service": "GigInsure API", "docs": "/docs"}

# Include router and middleware
app.include_router(api_router)

# Build allowed origins list: supports multiple comma-separated URLs in FRONTEND_URL
